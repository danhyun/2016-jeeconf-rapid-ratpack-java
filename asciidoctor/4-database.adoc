...
In order to provide data persistence to our REST application we'll make use of a number of libraries.
...
We'll use this API and h2 to tell jOOQ how to connect to our datasource, which schemata/tables to include and where to send the generated files.
...
We need to register the H2 datasource, the Ratpack HikariCP module with Ratpack's registry.
...
.{java}/App2.java
----
include::../{java}/App2.java[tags=jooq]
----
